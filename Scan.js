...
export default class Scan {
    ...
    static startFeed (feedTarget) {
        ...
        data.mw = 480;
        data.mh = 360;
        data.image = "data:image/gif;base64,R0lGODlhAQABAAAAACH5BAEKAAEALAAAAAABAAEAAAICTAEAOw==";
        ...
        OS.choosecamera('back', Camera.flip);
    ...
    static cameraToolsOn () {
        ...
        const captureContainer = newHTML('div', 'right-snapshot-container', scanFrame);
        captureContainer.setAttribute('id', 'capture-container');
        // const capture = newHTML('div', 'snapshot', captureContainer);
        const capture = newHTML('div', 'right-snapshot', captureContainer);
    ...
    static captureSnapshot (e) {
        e.stopPropagation();
        e.preventDefault();
        OS.captureimage('Scan.displayimage');
    }
    ...
    static displayimage (str) {
        console.log('displayimage');
        OS.stopfeed();
        Scan.secondaryToolsOn();
        if (str !== 'error getting a still') {
            ...
            img.addEventListener('load', e => {
                ...
                const w = window.screen.availWidth;
                const h = window.screen.availHeight;
                const resw = img.naturalWidth;
                const resh = img.naturalHeight;
                const r = (3/4);
                const dx = (w - r*w)/2;
                const dy = (h - r*h)/2;
                const dw = r * w;
                const dh = dw * (3/4);
                ctx.drawImage(img, 0, 0, resw, resh, dx, dy, dw, dh);
    ...
    static secondaryToolsOn () {
        Scan.cameraToolsOff();
        console.log('secondaryToolsOn');
        // scanFrame.setAttribute('class', 'modal-backdrop fade dark');
        // setProps(scanFrame.style, {
        //     display: 'block'
        // });
        const newbar = newHTML('div', 'secondarytopbar', scanFrame);
        newbar.setAttribute('id', 'secondarycontrols');
        
        const checkmark = newHTML('div', 'cameracheck', newbar);
        checkmark.setAttribute('id', 'cameracheck');
        checkmark.setAttribute('key', 'cameracheck');
        // if (isAndroid && !AndroidInterface.scratchjr_has_multiple_cameras()) {
        //     fc.style.display = 'none';
        // }
        checkmark.ontouchstart = Scan.checkMark;
        checkmark.onmousedown = Scan.checkMark;

        console.log('got here');
       
        const camundo = newHTML('div', 'cameraundo', newbar);
        camundo.setAttribute('id', 'cameraundo');
        camundo.ontouchstart = Scan.undopicture;
        camundo.onmousedown = Scan.undopicture;
        console.log('cam tools on finish');
        console.log(this.script);
    }
    ...
    static secondaryToolsOff () {
        console.log('secondarytoolsoff');
        if (gn('secondarycontrols')) {
            gn('secondarycontrols').parentNode.removeChild(gn('secondarycontrols'));
        }
    }
    ...
    static processimage (e) {
        ...
        for (let i = 0; i < stripArray.length; i++) {
            ...
            if (!old_ids.includes(id)) {
                ...
                strip.push([arucoDict[id], p, 400, 65]);
                old_ids.push(id);
            }           
        }
